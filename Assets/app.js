(()=>{'use strict';
const sections=['companies','wallets','fleet','market','deliveries','contracts','industry','maintenance','financing','assignments','yardPlans','diagnostics'];
const labels={companies:'Companies',wallets:'Wallets',fleet:'Fleet',market:'Catalog',deliveries:'Deliveries',contracts:'Contracts',industry:'Industry',passengers:'Passengers',maintenance:'Maintenance',financing:'Financing',assignments:'Assignments',yardPlans:'Yard plans',diagnostics:'Diagnostics'};
const sectionHelp={
...
const emptyMessages={
  ...
  market:'No market offer exists yet. Offers are prepared during active gameplay once a delivery depot is available. If the stock is sold out, check again later or ask the host.',deliveries:'No rolling-stock delivery is pending.',
...
class ManagementApp{
  constructor(root,transport){this.root=root;this.transport=transport;this.snapshot=null;this.activeSection='companies';this.lastEnvelope=null;this.status=document.createElement('p');this.status.className='bdvm-management__status';root.classList.add('bdvm-management');root.append(this.status)}
