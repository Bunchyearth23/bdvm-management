(()=>{'use strict';
const sections=['companies','wallets','fleet','market','leases','assignments','financing','yardPlans','industry'];
const labels={companies:'Compagnies',wallets:'Portefeuilles',fleet:'Flotte',market:'Catalogue',leases:'Locations',assignments:'Missions',financing:'Financement',yardPlans:'Triage',industry:'Industrie'};
...
class ManagementApp{
  ...
  async refresh(){this.setStatus('Loading','Loading…');try{this.snapshot=await this.transport.snapshot();this.render();this.setStatus('Succeeded',`Résultat réconcilié — state autoritaire v${this.snapshot.version??'?'} — ${this.snapshot.correlationId??''}`)}catch(error){this.setStatus('Offline',`Offline — ${error.message}`)}}
